...
window.scroll(newX,newY)},1000/60)};$(function(){var container=$('.whatsapp__button__container');var active='whatsapp__button__container--active';$('#whatsapp').click(function(e){e.stopPropagation();container.toggleClass(active);if(container.hasClass(active)){$(document).click(function(){container.removeClass(active)});console.log('clicou')}})});const screenPosition=window.innerHeight/2;function animationAboutUs(){const aboutUs=document.querySelector('.aboutUs');const aboutUsPosition=aboutUs.getBoundingClientRect().top;if(aboutUsPosition<screenPosition){aboutUs.classList.add('aboutUs--animation')}}
window.addEventListener('scroll',animationAboutUs);function animationDNA(){const dna=document.querySelector('.dna');const dnaPosition=dna.getBoundingClientRect().top;if(dnaPosition<screenPosition){dna.classList.add('dna--animation')}}
window.addEventListener('scroll',animationDNA);function animationSteps(){const steps=document.querySelector('.projectSteps');const stepsPosition=steps.getBoundingClientRect().top;if(stepsPosition<screenPosition){steps.classList.add('projectSteps--animation')}}
window.addEventListener('scroll',animationSteps);function animationProjects(){const projects=document.querySelector('.projects');const projectsPosition=projects.getBoundingClientRect().top;if(projectsPosition<screenPosition){projects.classList.add('projects--animation')}}
window.addEventListener('scroll',animationProjects);function animationTestimonial(){const testimonial=document.querySelector('.testimonial');const testimonialPosition=testimonial.getBoundingClientRect().top;if(testimonialPosition<screenPosition){testimonial.classList.add('testimonial--animation')}}
window.addEventListener('scroll',animationTestimonial);function animationAwards(){const awards=document.querySelector('.awards');const awardsPosition=awards.getBoundingClientRect().top;if(awardsPosition<screenPosition){awards.classList.add('awards--animation')}}
window.addEventListener('scroll',animationAwards);function animationContact(){const contact=document.querySelector('.contact');const contactPosition=contact.getBoundingClientRect().top;if(contactPosition<screenPosition){contact.classList.add('contact--animation')}}
window.addEventListener('scroll',animationContact);function animationOffice(){const office=document.querySelector('.office');const officePosition=office.getBoundingClientRect().top;if(officePosition<screenPosition){office.classList.add('office--animation')}}
...
var head=document.head||document.getElementsByTagName('head')[0];var style=document.createElement('style');style.type='text/css';head.appendChild(style);if(style.styleSheet){style.styleSheet.cssText=css}else{style.appendChild(document.createTextNode(css))}};injectCSS(styles);return sweetAlert$1})));if(typeof window!=='undefined'&&window.Sweetalert2)window.sweetAlert=window.swal=window.Sweetalert2;$(document).ready(function(){const path=$(location).attr("origin");const file='wp-content/themes/stdarquitetura/source/Support/Sender.php';$('#form').submit(function(e){e.preventDefault();$(".contact__form__content__status").removeClass("d-none");$.ajax({url:path.concat('/').concat(file),type:'POST',cache:!1,data:$('#form').serialize(),success:function(data){$('.contact__form__content__status').append(data);setTimeout(function(){$(".contact__form__content__status").addClass("d-none")},3000)},error:function(){$('.contact__form__content__status').append('Erro')}});return!1})})