...
window.scroll(newX,newY)},1000/60)};const screenPosition=window.innerHeight/2;function animationAboutUs(){const aboutUs=document.querySelector('.aboutUs');const aboutUsPosition=aboutUs.getBoundingClientRect().top;if(aboutUsPosition<screenPosition){aboutUs.classList.add('aboutUs--animation')}}
...
var head=document.head||document.getElementsByTagName('head')[0];var style=document.createElement('style');style.type='text/css';head.appendChild(style);if(style.styleSheet){style.styleSheet.cssText=css}else{style.appendChild(document.createTextNode(css))}};injectCSS(styles);return sweetAlert$1})));if(typeof window!=='undefined'&&window.Sweetalert2)window.sweetAlert=window.swal=window.Sweetalert2;$(document).ready(function(){$('#form').submit(function(){$.ajax({url:'https://localhost/stdarquitetura.com.br/wp-content/themes/stdarquitetura/source/Support/Sender.php',type:'POST',data:$('#form').serialize(),success:function(data){$('.contact__form__content__status').html(data)}});return!1})});$(function(){$("#button").click(function(){$(".contact__form__content__status--loading").removeClass("d-none")})})